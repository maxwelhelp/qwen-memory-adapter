# Эксперимент 2e: exp2d + ОБУЧАЕМАЯ η-ГОЛОВА (surprise-гейт) + ПОРОГ ЗАПИСИ.
...
def main():
    ...
    # ---- ФАЗА 1.5: η-ГОЛОВА — «что писать» (учитель mask_secret: секрет=1, мусор=0) ----
    print("фаза 1.5: η-голова (surprise-гейт) ...", flush=True)
    ...
    for ep in range(8):
        ...
        for i, d in enumerate(train):
            ...
            mask = d["mask_secret"].to(DEV)[:-1]     # парные токены (k: 0..n-2)
    ...
    torch.save(model.state_dict(), "exp2e_slots_assoc_eta.pt")
    print("Сохранено: exp2e_slots_assoc_eta.pt", flush=True)
