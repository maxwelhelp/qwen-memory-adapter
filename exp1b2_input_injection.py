# Эксперимент 1b2 (PLAN_MEMORY_V3, Ф1-вариант B, итерация 2): ВЕКТОРНАЯ ИНЪЕКЦИЯ во ВХОД последнего слоя.
...
# Итерация 2: лосс = MSE(mix, target_in) + 0.5·CE + 0.1·MSE(h_out', h_outB).
# oracle-тест показал: mix ≈ target_in (норма ~36) ⇒ правильный ответ (acc6 1.00);
# наивный MSE на выходе раздувал масштаб mix (160-190 вместо 36) — чиним прямой регрессией.
...
def main():
    ...
    for ep in range(EPOCHS_D):
        ...
        for i, d in enumerate(train):
            ...
            loss_mix = lossf(mv, d["target_in"].to(DEV))   # ГЛАВНОЕ: mix ≈ target_in
            loss_ce = cef(cand_logits(h_out_p), tgt)
            loss_out = lossf(h_out_p, y_out)
            loss = loss_mix + 0.5 * loss_ce + 0.1 * loss_out
            loss.backward()
            tot_mix = loss_mix.item()
            tot += loss.item(); n += 1
            if (i + 1) % BATCH == 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                opt.step(); opt.zero_grad()
        ...
        print(f"эпоха {ep:2d}: train {tot / n:.4f} (mix-часть {tot_mix / n:.4f}) | val_mse {v_mse / nv:.4f} | "
              f"acc6 {a6 / nv:.2f} | acc_open {a_open / nv:.2f} | "
              f"‖mix−Δ_in‖ {v_d / nv:.4f} | g={model.g().item():.3f} | {pt}", flush=True)

    torch.save(model.state_dict(), "exp1b2_memory_injection.pt")
