# Эксперимент 2d: СЛОТЫ + АССОЦИАТИВНАЯ ЗАПИСЬ + RETRIEVAL-ЧТЕНИЕ (синтез).
...
class SlotsAssocRetrieval(nn.Module):
    ...
    def write_work(self, h_ctx):
        """ассоциативная запись по слотам: k=W_k(h_t), v=W_v(h_{t+1}); шаги нормированные"""
        ...
        for c in range(0, len(k), CHUNK):
            ...
            for s, items in groups.items():
                ...
                for _ in range(ITERS):
                    ...
                    iloss = F.mse_loss(pred, vvs)          # АССОЦИАТИВНАЯ запись M(k)≈v
...
def main():
    ...
    print("фаза 2: слоты+ассоциативная запись+retrieval-чтение ...", flush=True)
    ...
    torch.save(model.state_dict(), "exp2d_slots_assoc_retrieval.pt")
    print("Сохранено: exp2d_slots_assoc_retrieval.pt", flush=True)
