...
def main():
    ...
    from train_memory_distill import tok
    for d in data:
        d["secret_tok0"] = torch.tensor(tok.encode(d["secret"], add_special_tokens=False)[0])
    ...
    for ep in range(PHASE2_EPOCHS):
        ...
        model.eval()
        va_open, v_mse, vd, nv = 0, 0.0, 0.0, 0
        per_type = {}
        for d in val:
            ctx = d["ctx_hidden"].to(DEV)
            q_h = d["q_hidden"].to(DEV)
        ...
        print(f"  val: acc_open = {va_open / nv:.2f} (шанс ~0.03) | "
