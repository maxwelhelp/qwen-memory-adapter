# Эксперимент 2h-FULL: полный масштаб (N_TRAIN=320, EPOCHS=40) рабочей схемы exp2h.
# Одна переменная vs exp2h: масштаб. Ожидание: межсессионный тест > 36/40.
...
def main():
    ...
    for ep in range(PHASE2_EPOCHS):
        ...
        for d in val:
            ...
            work = model.write_work(ctx)          # ВНЕ no_grad (клоны)
            with torch.no_grad():
                mv = model.mix6(q_h, work)
    ...
    torch.save(model.state_dict(), "exp2h_full.pt")
    print("Сохранено: exp2h_full.pt", flush=True)
