# Эксперимент 6b: атомы + МЯГКОЕ ЧТЕНИЕ (без жёсткого top-k).
# exp6: чтение top-8 из 384 атомов — записанные атомы не читаются → сигнал в gate_key
# нулевой с нуля (acc6 0.23). Здесь: чтение = softmax-гейт по ВСЕМ атомам (как exp5:
# softmax по слотам), top-k остаётся только на ЗАПИСИ. Одна переменная vs exp6.
#
...
class AtomBank(nn.Module):
    ...
    def read_atoms(self, write_buf, q):
        """МЯГКОЕ чтение: softmax-гейт по всем атомам (сигнал всегда есть, как exp5).
        Дифференцируемо по gate_key. Возвращает (вектор, confidence retrieval)."""
        g = self.gate(q)
        gm = torch.softmax(g, dim=0)          # без жёсткого top-k
        out = (gm.unsqueeze(-1) * write_buf).sum(0)
        conf = gm.abs().sum()                 # ≈1 при softmax — слабый сигнал; оставляем
        return out, conf
