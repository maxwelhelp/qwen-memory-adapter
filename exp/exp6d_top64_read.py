# Эксперимент 6d: exp6c + ЧТЕНИЕ top-64 (компромисс).
# exp6c: синтетика cos 1.00, но реальные данные — шанс: чтение top-8 из 384 не
# пересекается с записанными атомами → сигнал в key нулевой → холодный старт
# замкнут (нечем выучить соответствие вопрос↔ключ). Здесь: чтение top-64
# (сигнал проходит — как dense exp5), запись остаётся top-8. Одна переменная.
#
# ДИАГНОЗ exp6b (почему было acc6 45% / acc_open 15%, хуже линейных слотов 85-100%):
# синтетический тест (20 фактов, 128 атомов, top_k=8, noise=0 — точное совпадение
# query с записанным ключом) дал cos(retrieved, target) = 0.248 для схемы exp6b —
# то есть retrieval НЕ восстанавливал записанное ДАЖЕ в идеальных условиях.
# Причины (обе исправлены здесь):
#   1. gate(x) = SiLU(gate_key @ x) — при записи считался от k, при чтении от q,
#      НЕЛИНЕЙНОСТЬ на скоре (SiLU немонотонна в области z<0, см. минимум ≈-0.278
#      при z≈-1.278) может дать q, близкий к k, совсем другую активацию, чем сам k.
#      Похожесть входа не гарантированно переходит в похожесть отклика.
#   2. read_atoms делал softmax по ВСЕМ N_ATOMS=384 атомам, хотя записано (top-k)
#      было только ~8 — 376 "пустых" атомов разбавляли сигнал даже при удачном gate.
#
# ИСПРАВЛЕНИЕ (проверено на синтетике: cos=0.913 при noise=0 — вровень с линейной
# ассоциативной памятью cos=0.912):
#   - Один и тот же ключ `self.key` [N_ATOMS, D] используется И на запись, И на
#     чтение (было: gate_key, тоже один параметр — это сохранено).
#   - Скор = СЫРОЕ скалярное произведение key @ x, БЕЗ SiLU/ReLU на самом скоре.
#     Селективность обеспечивается top-k + softmax, а не поэлементной нелинейностью.
#     (Нелинейность в схеме не пропадает совсем — она в DynamicGate, ниже.)
#   - Запись: top-k(score, K) по k → softmax ТОЛЬКО среди top-k → delta-rule.
#   - Чтение: top-k(score, K) по q (тот же принцип, что на записи) → softmax
#     ТОЛЬКО среди top-k → взвешенная сумма buf[idx]. Никакого dense-softmax
#     по всем 384 атомам — sparse-to-sparse, без разбавления пустыми.
#   - confidence больше не вырожден (было: gm.abs().sum()≈1 всегда, толку ноль) —
#     теперь это сумма top-k весов ДО нормировки (реальная сила совпадения query
#     с ближайшими ключами: высокая, если query явно похож на один из ключей,
#     низкая, если размазан по многим слабо).
#
# Это НЕ откат к "тупым линейным слотам" — атомов на порядок больше (384 vs 32),
# адресация тоньше (top-k из 384 вместо argmax в 1 из 32), т.е. должно снимать
# именно ту проблему (грубая адресация → "пароль от почты"/"пароль от телефона"
# конфликтуют в одном слоте), ради которой атомы и затевались. Но retrieval теперь
# математически такой же по духу, как в линейном слое — просто разреженный.
#
# ДВЕ ФАЗЫ ОБУЧЕНИЯ (без изменений от exp6b):
...
#     W_q, W_out, key, DynamicGate. write_buf НЕ обучается backprop'ом — он
...
TOP_K = 8
TOP_K_READ = 64   # чтение шире записи (холодный старт)               # разреженная активация: сколько атомов реально трогаем за токен
CHUNK = 16
...
class AtomBank(nn.Module):
    """Банк rank-1 адресуемых атомов. `key` — ЕДИНЫЙ параметр для записи и чтения
    (тот же принцип, что и в exp6b: чтение дифференцируемо -> ключ обучается).
    ГЛАВНОЕ ОТЛИЧИЕ от exp6b: скор = сырое скалярное произведение key@x (без
    поэлементной нелинейности типа SiLU на самом скоре), и top-k ОДИНАКОВО
    применяется и на записи, и на чтении (sparse-to-sparse, не sparse-to-dense).
    См. синтетический тест в диалоге: это восстанавливает cos≈0.91 (как линейная
    ассоциативная память) вместо cos≈0.25 у SiLU+dense-softmax варианта."""
    def __init__(self, d=D, n_atoms=N_ATOMS, top_k=TOP_K):
        super().__init__()
        self.d, self.n_atoms, self.top_k = d, n_atoms, top_k
        self.key = nn.Parameter(torch.randn(n_atoms, d) * (d ** -0.5))

    def score(self, x):
        """Сырой скор похожести x на все атомы: [D] -> [n_atoms].
        БЕЗ нелинейности на самом скоре (см. диагноз exp6b в шапке файла) —
        дифференцируемо по key и по x."""
        return self.key @ x

    def topk_softmax(self, s, k=None):
        """top-k индексы + softmax-веса ТОЛЬКО среди них (не по всем n_atoms)."""
        if k is None:
            k = self.top_k
        k = min(k, self.n_atoms)
        idx = torch.topk(s, k).indices
        w = torch.softmax(s[idx], dim=0)
        return idx, w

    def init_buf(self):
        """'θ0' — пустая память на старте сессии/примера."""
        return torch.zeros(self.n_atoms, self.d, device=self.key.device)
    ...
    def write_atoms(self, write_buf, k_seq, v_seq, eta_t):
        ...
        # попытка обучать запись через buf-граф дала взрыв (4-е подтверждение
        # нестабильности bilevel в этой схеме). НО: key ОБУЧАЕТСЯ через ЧТЕНИЕ
        # (read_atoms внутри mix_vec дифференцируемо) — тот же параметр, что и запись,
        # так что улучшение чтения автоматически улучшает адресацию записи.
        with torch.no_grad():
            for t in range(len(k_seq)):
                if eta_t[t] < ETA_MIN_SKIP:
                    continue
                s = self.score(k_seq[t])                # [n_atoms], сырой скор (запись)
                idx, w = self.topk_softmax(s)            # top-k атомов + нормированные веса
                strength = (ETA_WRITE * eta_t[t] * w).unsqueeze(-1)   # [k,1]
                decay = GAMMA
                buf[idx] = buf[idx] * (1 - decay) + strength * (v_seq[t].unsqueeze(0) - buf[idx])
        return buf

    def read_atoms(self, write_buf, q):
        """СПАРС-чтение: top-k по q (тот же принцип, что при записи) + softmax
        ТОЛЬКО среди top-k. Никакого dense-softmax по всем n_atoms — исключает
        разбавление ~90% пустых/нерелевантных атомов. Дифференцируемо по key."""
        s = self.score(q)
        idx, w = self.topk_softmax(s, k=TOP_K_READ)   # ЧТЕНИЕ top-64
        out = (w.unsqueeze(-1) * write_buf[idx]).sum(0)
        # РЕАЛЬНАЯ сила совпадения (в exp6b conf≈const≈1, был бесполезен):
        # чем выше максимальный сырой скор среди top-k, тем увереннее retrieval.
        # tanh — просто чтобы держать признак в [-1,1] для DynamicGate (score
        # неограничен по масштабу, в отличие от softmax-весов).
        conf = torch.tanh(s[idx].max())
        return out, conf
...
class AtomMemory(nn.Module):
    ...
    def write_work(self, h_ctx, entropy=None):
        ...
        (без no_grad) — key_pool/W_k/W_v/atoms.key получают градиент через mix_vec→loss."""
        if entropy is None:
