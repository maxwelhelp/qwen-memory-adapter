# Эксперимент 2a: TITANS-КОРРЕКТНАЯ память (по коду neural_memory.py, не по памяти о статье).
...
class TitansCorrect(nn.Module):
    ...
    def write(self, h_ctx):
        ...
        k = F.gelu(self.W_k(h_ctx))
        v = F.gelu(self.W_v(h_ctx))
...
def main():
    ...
    torch.save(model.state_dict(), "exp2a_titans_correct.pt")
    print("Сохранено: exp2a_titans_correct.pt", flush=True)
